...
$.dropPanel = {
	...
	open: function (options) {
		// default, pic, login
		var op = $.extend({type: 'GET', url: '', data: {}, html: '', callback: null}, options);
		var $box = this.$box;
		...
		if (op.url) {
			...
			if (op.callback) {
				op.callback.call($box, op.html, $.extend(params, op.data));
			} else {
				$box.html(op.html).initUI();
			}
		}
