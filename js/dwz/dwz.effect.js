...
(function ($) {
	...
	$.extend({
		...
		colorRgba: function (options) {
			var op = $.extend({color: '#ffffff', opacity: 1}, options);

			var regHex = /^#([0-9a-fA-f]{3}|[0-9a-fA-f]{6})$/;// 16进制颜色值的正则
			var regRgb = /^(rgb\(|RGB\()/; // rgb颜色值的正则
			var color = op.color.toLowerCase();// 把颜色值变成小写
			if (regHex.test(color)) {
				...
				// 把RGB的3个数值变成数组
				var colorChange = color.replace(/(?:\(|\)|rgb|RGB)*/g, "").split(",");
				return "rgba(" + colorChange.join(",") + "," + op.opacity + ")";
			} else {
				return color;
			}
	...
	$.fn.extend({
		...
		fadeOut: function (duration, animationend) {
			return this.effect([{
				className: 'fadeOut animated', after: function () {
					this.hide()
				}
			}], animationend);
		},
		fadeIn: function (duration, animationend) {
			return this.effect([{
				className: 'fadeIn animated', before: function () {
					this.show()
				}, after: function () {
					this.show()
				}
